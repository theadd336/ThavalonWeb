...
use super::{LobbyChannel, LobbyResponse};
...
impl PlayerClient {
    pub fn new(
        ...
        from_client: SplitStream<WebSocket>,
    ) -> Self {
    ...
    fn spawn_stable_tasks(
        ...
        let game_to_outbound_task = self.to_outbound_task.clone();
        task::spawn(async move {
            while let Some(game_msg) = from_game.recv().await {
                ...
                // stable.
                let _ = game_to_outbound_task.send(value).await;
                dw
            }
        });
            ...
            while let Some(outbound_msg) = outbound_task_rx.recv().await {
                ...
                match outbound_msg {
                    OutboundTaskMessageType::ToClient(msg) => {
                        log::debug!("Sending message {} to client {}.", msg, self.client_id);
                        ...
                        log::debug!("Successfully sent message to client {}.", self.client_id);
                    }

                    OutboundTaskMessageType::NewWebSocket(ws) => {
                        log::info!("Received new connection for client {}.", self.client_id);
                        to_client = ws;
                    }
                }
            }
    ...
    fn spawn_from_client_task(&mut self, from_client: SplitStream<WebSocket>) {
        log::info!(
        ...
        task::spawn(async move {
            while let Some(incoming_msg) = from_client.next().await {
                if let Err(e) = incoming_msg {
                    log::error!("An error occurred while reading messages from the incoming connection for client {}. {}", self.client_id, e);
                    return;
                }
