...
use super::{Game, GameSpec, Card, PlayerId};
...
async fn run_game<I: Interactions>(game: &Game, interactions: &mut I) -> Result<(), GameError> {
    ...
    log::debug!("{} and {} are proposing mission 1", game.name(first_proposer), game.name(second_proposer));
    ...
    'missions: for mission in 2..=5 {
        'proposals: for proposal_num in 1..=game.spec.proposals() {
            ...
            let proposal = ge.get_proposal(proposer, mission, proposal_num as u8).await?;
...
impl <'a, I: Interactions> GameEngine<'a, I> {
    fn new(game: &'a Game, interactions: &'a mut I) -> GameEngine<'a, I> {
        let proposers = game.proposal_order()
        .iter()
        .cloned()
        .cycle()
        ...
        GameEngine { game, interactions, proposers }
    }

    async fn get_proposal(&mut self, from: PlayerId, mission: u8, proposal: u8) -> Result<Proposal<'a>, GameError> {
        log::debug!("Getting a proposal from {} for mission {}, proposal {}", self.game.name(from), mission, proposal);
        self.interactions.send(Message::NextProposal {
            proposer: from,
            mission,
            proposal
        }).await?;
        ...
        let players = self.interactions.receive(|player, action| {
            if player != from {
                Err("It's not your proposal!".to_string())
            } else {
                match action {
                    Action::Propose { players } if players.len() == size => Ok(players),
                    Action::Propose { .. } => Err(format!("Proposal must contain {} players", size)),
                    _ => Err("You can't do that right now".to_string())
                }
            }
        }).await?;

        self.interactions.send(Message::ProposalMade {
            proposer: from,
    ...
    async fn vote(&mut self) -> Result<VotingResults, GameError> {
            ...
            self.interactions.receive(|player, action| {
                match action {
                    Action::Vote { upvote } => match votes.entry(player) {
                        ...
                        Entry::Vacant(entry) => {
                            entry.insert(upvote);
                            log::debug!("{} {}", game.name(player), if upvote { "upvoted" } else { "downvoted" });
                            Ok(())
                        }
                    },
                    _ => Err("You can't do that right now".to_string()),
        ...
        self.interactions.send(Message::VotingResults {
    ...
    async fn send_mission(&mut self, proposal: Proposal<'_>, mission: usize) -> Result<bool, GameError> {
                ...
                if proposal.players.contains(&player) {
                    match action {
                        Action::Play { card } => match cards.entry(player) {
                            Entry::Occupied(entry) => Err(format!("You already played a {}", entry.get())),
                            Entry::Vacant(entry) => {
        ...
        log::debug!("Mission {} {}", mission, if failed { "failed" } else { "passed" });

        self.interactions.send(Message::MissionResults {
            passed: !failed,
...
fn is_failure<'a, I: IntoIterator<Item=&'a Card>>(spec: &GameSpec, mission: usize, cards: I) -> bool {
    let (fails, reverses) = cards.into_iter().fold((0, 0), |(fails, reverses), card| {
        match card {
...
impl VotingResults {
    ...
    fn upvotes<'a>(&'a self) -> impl Iterator<Item = PlayerId> + 'a {
        self.votes.iter().filter_map(|(player, vote)| if *vote { Some(*player) } else { None })
    }
...
impl <'a> fmt::Display for Proposal<'a> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        self.players.iter().format_with(", ", |id, f| f(&self.game.name(*id))).fmt(f)?;
...
fn test_is_failure() {
    ...
    assert!(!is_failure(spec, 1, &[Card::Success, Card::Success, Card::Success]));
    assert!(!is_failure(spec, 1, &[Card::Success, Card::Fail, Card::Reverse]));
    assert!(!is_failure(spec, 1, &[Card::Success, Card::Reverse, Card::Reverse]));

    assert!(is_failure(spec, 1, &[Card::Success, Card::Success, Card::Fail]));
    assert!(is_failure(spec, 1, &[Card::Success, Card::Fail, Card::Fail]));
    assert!(is_failure(spec, 1, &[Card::Fail, Card::Fail, Card::Fail]));

    assert!(is_failure(spec, 1, &[Card::Success, Card::Success, Card::Reverse]));
    assert!(is_failure(spec, 1, &[Card::Fail, Card::Reverse, Card::Reverse]));

    // TODO: replace with proper 7-player spec
    let mut with_seven = spec.clone();
    with_seven.double_fail_mission_four = true;

    assert!(!is_failure(&with_seven, 4, &[Card::Success, Card::Success, Card::Success, Card::Fail]));
    assert!(!is_failure(&with_seven, 4, &[Card::Success, Card::Reverse, Card::Fail, Card::Fail]));
    assert!(!is_failure(&with_seven, 4, &[Card::Success, Card::Success, Card::Success, Card::Reverse]));
    assert!(!is_failure(&with_seven, 4, &[Card::Reverse, Card::Reverse, Card::Fail, Card::Success]));
    
    assert!(is_failure(&with_seven, 4, &[Card::Success, Card::Success, Card::Fail, Card::Fail]));
    // This is why reversing mission 4 as Lance is a choice
    assert!(is_failure(&with_seven, 4, &[Card::Success, Card::Success, Card::Reverse, Card::Fail]));
    assert!(is_failure(&with_seven, 4, &[Card::Reverse, Card::Reverse, Card::Fail, Card::Fail]));
}
...
mod test {
    ...
    use super::*;
    use super::super::{Players, Player, Role};
    ...
    fn test_proposals() {    
        ...
        engine.interactions.extend_actions(vec![
            (1, Action::Propose { players: hashset![1, 3] }),
            (2, Action::Propose { players: hashset![2, 4, 1] }),
            (2, Action::Propose { players: hashset![2, 4] }),
        ]);
        ...
        assert_eq!(interactions.messages_for(1).collect::<Vec<_>>(), vec![
            &Message::Error("It's not your proposal!".to_string())
        ]);

        assert_eq!(interactions.messages_for(2).collect::<Vec<_>>(), vec![
            &Message::Error("Proposal must contain 2 players".to_string())
        ]);

        assert_eq!(interactions.broadcasts(), &[
            Message::NextProposal { proposer: 2, mission: 1, proposal: 1 },
