...
use super::PlayerId;
use super::runner::{Action, Message, GameError};
...
pub(super) trait Interactions {
    ...
    async fn receive<F, R>(&mut self, mut f: F) -> Result<R, GameError> where F: FnMut(PlayerId, Action) -> Result<R, String>;
}
...
impl ChannelInteractions {
    ...
    pub fn add_player(&mut self, id: PlayerId, incoming: mpsc::Receiver<Action>, outgoing: mpsc::Sender<Message>) {
...
impl Interactions for ChannelInteractions {
    async fn send_to(&mut self, player: PlayerId, message: Message) -> Result<(), GameError> {
        self.outbox.get_mut(&player).unwrap().send(message).await.map_err(|_| GameError::PlayerUnavailable { id: player })
    }

    async fn send(&mut self, message: Message) -> Result<(), GameError> {
        let sends = self.outbox.iter_mut().map(|(id, sender)| {
            sender.send(message.clone()).map_err(move |_| GameError::PlayerUnavailable { id: *id })
        });
...
pub(super) mod test {
    ...
    use super::super::runner::{Message, Action, GameError};
    ...
    impl TestInteractions {
        ...
        pub fn messages_for(&self, player: PlayerId) -> impl Iterator<Item=&Message> {
            self.messages.iter().filter_map(move |(recipient, message)| {
                if recipient == &player {
                    Some(message)
                } else {
                    None
